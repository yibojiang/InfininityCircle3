...
private function UpdateFunction () {
	...
	//Mathf.Abs(currentMovementOffset.x)>0 ||
	if ( Mathf.Abs(currentMovementOffset.x)>0)
	{
		//xToggle+=Time.deltaTime*0.2;
		
		xToggle+=Time.deltaTime*4;
		cameraTransform.localPosition.y=playerHeight-0.08+0.08*Mathf.Sin(xToggle);
		
		
		if (Mathf.Sin(xToggle)<-0.9)
		{
			if (!foot.audio.isPlaying)
			{
				foot.audio.Play();
			}
		}
	...
	{
		cameraTransform.localPosition.y+=(playerHeight-cameraTransform.localPosition.y)*Time.deltaTime;
		
		if (xToggle>0)
		{
			xToggle=0;
			if (foot.scene==FootStepEnum.Wood)
			{
				foot.audio.PlayOneShot(foot.woodFootStepEnd);
			}
		}
	}
